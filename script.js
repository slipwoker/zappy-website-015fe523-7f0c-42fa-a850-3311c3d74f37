document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function getFieldError(field) {
      ...
      if (field.minLength > 0 && value.length < field.minLength && value !== '') {
        return (field.dataset.errorMinlength || 'Minimum {min} characters required.').replace('{min}', field.minLength);
      }
    ...
    function getFormFields() {
      return Array.from(contactForm.querySelectorAll('input, textarea, select'))
  ...
  const ANIMATION_CLASS    = 'fade-in';
  const ANIMATED_CLASS     = 'animated';
  const ANIMATION_SELECTOR = '.animate-on-scroll, [data-animate]';

  const animatableElements = Array.from(document.querySelectorAll(ANIMATION_SELECTOR));

  if (animatableElements.length > 0) {

    // Assign staggered delays for grouped children
    animatableElements.forEach(function (el, index) {
      if (!el.style.transitionDelay && !el.dataset.delay) {
        const parent      = el.parentElement;
        const siblings    = parent ? Array.from(parent.querySelectorAll(ANIMATION_SELECTOR)) : [];
    ...
    function checkAnimations() {
      animatableElements.forEach(function (el) {
        if (!el.classList.contains(ANIMATED_CLASS) && isInViewport(el)) {
          el.classList.add(ANIMATION_CLASS, ANIMATED_CLASS);
        }
      });
    }

    // Throttle with rAF
    let animationScrollTicking = false;
    window.addEventListener('scroll', function () {
      if (!animationScrollTicking) {
        requestAnimationFrame(function () {
          checkAnimations();
          animationScrollTicking = false;
        });
        animationScrollTicking = true;
      }
    }, { passive: true });
    ...
    if ('IntersectionObserver' in window) {
      ...
      const observer = new IntersectionObserver(function (entries) {
        entries.forEach(function (entry) {
          if (entry.isIntersecting && !entry.target.classList.contains(ANIMATED_CLASS)) {
            entry.target.classList.add(ANIMATION_CLASS, ANIMATED_CLASS);
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "בי לייזר אונליין משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '015fe523-7f0c-42fa-a850-3311c3d74f37',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '015fe523-7f0c-42fa-a850-3311c3d74f37';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '015fe523-7f0c-42fa-a850-3311c3d74f37';
